...
public class AppTransition implements Dump {
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(
                    Settings.CMREMIX.getUriFor(Settings.CMREMIX.ANIMATION_CONTROLS_EXIT_ONLY), false, this);
            resolver.registerContentObserver(
                    Settings.CMREMIX.getUriFor(Settings.CMREMIX.ANIMATION_CONTROLS_REVERSE_EXIT), false, this);
            for (int i = 0; i < 11; i++) {
                resolver.registerContentObserver(
    ...
    private void updateSettings() {
        ContentResolver resolver = mContext.getContentResolver();
        for (int i = 0; i < 11; i++) {
            mActivityAnimations[i] = Settings.CMREMIX.getInt(resolver,
                    Settings.CMREMIX.ACTIVITY_ANIMATION_CONTROLS[i], 0);
        }

        mExitOnly = Settings.CMREMIX.getBoolean(resolver,
                Settings.CMREMIX.ANIMATION_CONTROLS_EXIT_ONLY, true);
        mReverseExit = Settings.CMREMIX.getBoolean(resolver,
                Settings.CMREMIX.ANIMATION_CONTROLS_REVERSE_EXIT, false);

        mAnimationDuration = 15 * Settings.CMREMIX.getInt(resolver,
